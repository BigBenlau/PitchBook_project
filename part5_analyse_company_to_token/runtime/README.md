...
- `ARCHITECTURE.md`: human-readable architecture notes for the current queue-first runtime, with round mode retained as fallback.
...
- `scripts/5_run_queue_supervisor.py` is the active unattended queue-mode scheduler used by current longrun runs.
...
- Current unattended longrun semantics are queue-first:
  - `scripts/6_start_long_running_supervisor.py` launches detached runs and supports `--scheduler-mode round|queue`.
  - current production backlog runs use `--scheduler-mode queue`.
