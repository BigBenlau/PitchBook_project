...
DEFAULT_RUNTIME_POLICY_JSON = PART5_DIR / "runtime" / "policy.json"
...
def record_identity_override(
    ...
    if isinstance(samples, list) and len(samples) < 10:
        samples.append(
            f"{source}: task_index={task_index} company={company_name}: "
            + "; ".join(changes)
        )
...
def canonicalize_identity_fields(
    ...
    company_name = str(task.get("company_name", normalized.get("company_name", "")))
    changes: list[str] = []
    for key, expected in [
        ("task_index", task_index),
        ("company_id", str(task.get("company_id", normalized.get("company_id", "")))),
...
def repair_shifted_result_row(row: dict[str, str]) -> dict[str, str]:
    ...
    search_required = (repaired.get("project_search_required") or "").strip()
    search_reason = (repaired.get("project_search_reason") or "").strip()
    if search_required in {"light", "deep", "skip_candidate"} and search_reason in ALLOWED_YES_NO:
        repaired["project_search_required"], repaired["project_search_reason"] = search_reason, search_required

    project_url = (repaired.get("project_url") or "").strip()
    if project_url and not validate_json_list(project_url):
        has_token_evidence = (repaired.get("has_token_evidence") or "").strip()
        if has_token_evidence in {"", "[]"} and not project_url.startswith(("http://", "https://")):
            repaired["has_token_evidence"] = project_url
    ...
    if (repaired.get("status") or "").strip() != "completed" and (repaired.get("token_ticker") or "").strip() == "completed":
        repaired["status"] = "completed"
        if looks_like_iso_8601_utc(repaired.get("token_name", "")):
            repaired["completed_at"] = (repaired.get("token_name") or "").strip()
            repaired["token_name"] = "[]"
        repaired["token_ticker"] = "[]"
...
def validate_result_row(row: dict[str, str], source: Path) -> list[str]:
    ...
    if not has_token_evidence:
        errors.append("has_token_evidence is blank")
    if (row.get("project_search_required") or "").strip() == "yes":
        ...
        if has_token_evidence.lower() in {"yes", "no"}:
            errors.append("searched rows must summarize evidence in has_token_evidence, not bare yes/no")
    ...
    if token_count > 0:
        ...
        if not evidence_source_types:
            errors.append("token-positive rows require evidence_source_types")
        if has_token_evidence.lower() in {"", "no", "none", "false"}:
            errors.append("token-positive rows require affirmative has_token_evidence summary")
    if token_count == 0 and (row.get("project_search_required") or "").strip() == "yes":
        if has_token_evidence.lower() == "yes":
            errors.append("no-token searched rows cannot use bare yes in has_token_evidence")
    ...
    for schedule_row in schedule_rows:
        ...
        for row in rows:
            ...
            if requires_search_guarantee(schedule_row):
                ...
                if row.get("project_search_required") != "yes":
                    validation_errors.append(
                        f"{classifier_csv}: task_index={task_index}: search-guarantee rerun batches require project_search_required=yes"
                    )
...
def validate_row_order_against_tasks(
    ...
    for index in range(compare_count):
        ...
        if actual_task_index != expected_task_index:
            expected_company_name = str(task.get("company_name", ""))
            actual_company_name = (row.get("company_name") or "").strip()
            validation_errors.append(
                f"{results_csv}: row {index + 2}: expected "
                f"task_index={expected_task_index} company={expected_company_name!r}, found "
                f"task_index={actual_task_index} company={actual_company_name!r}"
            )
    return validation_errors
            ...
            if action == "edit_row":
                ...
                if corrected_row:
                    if expected_task:
                        ...
                        corrected_row["company_id"] = str(expected_task.get("company_id", ""))
                        corrected_row["company_name"] = str(expected_task.get("company_name", ""))
                    ...
                    expected_company_id = (
                        str(expected_task.get("company_id", ""))
                        if expected_task
                        else (row.get("company_id") or "")
                    )
                    if corrected_row.get("company_id") != expected_company_id:
                        validation_errors.append(
    ...
    for schedule_row in schedule_rows:
        ...
        for key, expected_header in [
            ...
            for index, row in enumerate(rows):
                ...
                if index < len(tasks):
                    task = tasks[index]
                    for field in ["task_index", "company_id", "company_name"]:
...
def main() -> None:
    ...
    if args.fail_on_identity_drift and identity_override_count and not args.allow_partial:
        validation_errors.append(
            ...
            "Workers must preserve task_index/company_id/company_name/normalized_domain exactly from tasks.jsonl."
        )
