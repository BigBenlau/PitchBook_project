...
from part6_schema import CLASSIFIER_CSV_COLUMNS, RESULT_CSV_COLUMNS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manage part6 round runtime: launch queues, attempt isolation, and policy-driven recovery.",
    )
...
def upgrade_row_to_attempt_architecture(row: dict[str, str]) -> dict[str, str]:
    ...
    next_company = str(tasks[prefix_len].get("investor_name", "")) if prefix_len < len(tasks) else ""
    ...
    if segment_end_index > 0 and segment_end_index <= len(tasks):
        ...
        segment_end_company = str(segment_end_task.get("investor_name", ""))
    else:
...
def build_launch_row(row: dict[str, str], inspection: dict[str, Any], policy: dict[str, Any]) -> dict[str, str]:
    ...
    if start_index >= len(tasks):
        ...
        start_company = str(tasks[start_index].get("investor_name", ""))
...
def build_spawn_prompt(row: dict[str, str], model: str, reasoning: str, launch_reason: str, attempt_mode: str) -> str:
    ...
    return (
        f"Use a fresh worker subagent for exactly one part6 batch attempt, then exit. Recommended model: {model}, reasoning {reasoning}.\n\n"
