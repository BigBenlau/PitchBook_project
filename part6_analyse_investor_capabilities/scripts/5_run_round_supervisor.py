...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the fixed part6 round harness: prepare -> launch -> mark-started -> "
...
def normalize_row_fields(row: dict[str, Any], fieldnames: list[str]) -> dict[str, str]:
    ...
    investor_archetype = normalized.get("investor_archetype", "")
    if investor_archetype == "invesment_or_holdings":
        normalized["investor_archetype"] = "investment_or_holdings"

    if fieldnames == RESULT_CSV_COLUMNS:
        for column in CAPABILITY_FLAG_COLUMNS:
            if normalized.get(column, "").strip() not in {"yes", "no"}:
                normalized[column] = "no"
        if not normalized.get("other_flags", "").strip():
            normalized["other_flags"] = "[]"
...
def task_window(tasks: list[dict[str, Any]]) -> tuple[str, str, str, str]:
    ...
    return (
        ...
        str(first.get("investor_name", "")),
        str(last.get("task_index", "")),
        str(last.get("investor_name", "")),
    )
    ...
    *,
    ...
    if existing_prefix_rows > 0 and existing_prefix_rows < len(shard_tasks):
        ...
        prefix_note = (
            ...
            f"- Continue from task_index {next_task.get('task_index', '')} ({next_task.get('investor_name', '')}) onward.\n"
