...
def build_worker_spawn_prompt(row: dict[str, str]) -> str:
    ...
    return f"""Use a fresh worker subagent for exactly one part6 batch attempt. Do not reuse a finished worker context for a later attempt or a later batch. Recommended model: gpt-5.4-mini, reasoning medium.
...
def build_verifier_spawn_prompt(row: dict[str, str]) -> str:
    return f"""Use a fresh verifier subagent for this part6 round. Recommended model: gpt-5.4-mini, reasoning high.
