...
def load_round_supervisor_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("part4_round_supervisor", module_path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the queue-based part4 harness: keep up to N active batch slots, "
            ...
            if not str(updated.get("started_at") or "").strip():
                ...
                for entry in registry:
                    ...
                    if ROUND.parse_int(entry.get("attempt_index"), 0) != current_attempt_index:
                        continue
                    if not ROUND.registry_entry_alive(entry):
                        continue
    ...
    try:
        with report_path.open("r", encoding="utf-8", newline="") as handle:
            for report_row in csv.DictReader(handle):
                ...
                if recommended_action in {"rerun_batch", "rerun_company"}:
...
def main() -> None:
    ...
    ROUND.install_supervisor_signal_handlers()
    ...
    while True:
        ...
        if queue_run_complete(schedule_rows, target_rounds) and not active_running_rows(schedule_rows, target_rounds, registry):
            ...
            ROUND.update_launcher_state_status(runs_dir, status="completed")
...
if __name__ == "__main__":
    try:
        main()
    except ROUND.SupervisorSignalExit as exc:
        ROUND.handle_supervisor_signal_exit(exc)
