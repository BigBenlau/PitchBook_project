...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect v2 part4 worker result CSVs, enforce verifier gates, and update final outputs.",
    )
    parser.add_argument(
        "--runs-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Final merged results CSV. Defaults to part4_analyse_token_to_company/agent_runs/token_company/results.csv.",
    )
    parser.add_argument(
        "--classifier-results-csv",
        type=Path,
        default=None,
        help="Final classifier/router CSV. Defaults to part4_analyse_token_to_company/agent_runs/token_company/classifier_results.csv.",
    )
    parser.add_argument(
        "--manual-review-csv",
        type=Path,
        default=None,
        help="Manual review CSV. Defaults to part4_analyse_token_to_company/agent_runs/token_company/needs_manual_review.csv.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Checkpoint JSON. Defaults to part4_analyse_token_to_company/agent_runs/token_company/checkpoint.json.",
    )
    parser.add_argument(
        "--allow-partial",
        action="store_true",
...
def record_identity_override(
    ...
    if isinstance(samples, list) and len(samples) < 10:
        samples.append(
            f"{source}: task_index={task_index} token={token_name}: "
            + "; ".join(changes)
        )
...
def canonicalize_identity_fields(
    ...
    token_name = str(task.get("token_name", normalized.get("token_name", "")))
    changes: list[str] = []
    for key, expected in [
        ("task_index", task_index),
        ("canonical_slug", str(task.get("canonical_slug", normalized.get("canonical_slug", "")))),
        ("token_symbol", str(task.get("token_symbol", normalized.get("token_symbol", "")))),
...
def repair_extra_columns(row: dict, fieldnames: list[str]) -> dict[str, str]:
    """Repair rows where unquoted commas split has_entity_evidence."""
    if None not in row:
    ...
    if "has_entity_evidence" not in fieldnames:
        return {field: row.get(field, "") for field in fieldnames}

    evidence_index = fieldnames.index("has_entity_evidence")
    tail_count = 4
    fixed_values = (
...
def repair_shifted_result_row(row: dict[str, str]) -> dict[str, str]:
    ...
    search_required = (repaired.get("entity_search_required") or "").strip()
    search_reason = (repaired.get("entity_search_reason") or "").strip()
    if search_required in {"light", "deep", "skip_candidate"} and search_reason in ALLOWED_YES_NO:
        repaired["entity_search_required"], repaired["entity_search_reason"] = search_reason, search_required

    project_url = (repaired.get("project_url") or "").strip()
    if project_url and not validate_json_list(project_url):
        has_entity_evidence = (repaired.get("has_entity_evidence") or "").strip()
        if has_entity_evidence in {"", "[]"} and not project_url.startswith(("http://", "https://")):
            repaired["has_entity_evidence"] = project_url
            repaired["project_url"] = "[]"

    if (repaired.get("project_name") or "").strip() in {"", "[]"}:
        status_value = normalize_json_list_string(repaired.get("status", ""))
        if validate_json_list(status_value):
            repaired["project_name"] = status_value

    completed_at_value = (repaired.get("completed_at") or "").strip()
    if validate_json_list(completed_at_value):
        project_url_value = normalize_json_list_string(repaired.get("project_url", ""))
        if project_url_value in {"", "[]"}:
            repaired["project_url"] = normalize_json_list_string(completed_at_value)
            repaired["completed_at"] = ""

    if (repaired.get("status") or "").strip() != "completed" and (repaired.get("mapped_entity_name") or "").strip() == "completed":
        repaired["status"] = "completed"
        if looks_like_iso_8601_utc(repaired.get("mapped_entity_legal_name", "")):
            repaired["completed_at"] = (repaired.get("mapped_entity_legal_name") or "").strip()
            repaired["mapped_entity_legal_name"] = "[]"
        repaired["mapped_entity_name"] = "[]"
        repaired["mapped_entity_type"] = "[]"
...
def validate_result_row(row: dict[str, str], source: Path) -> list[str]:
    ...
    if row.get("entity_search_required") == "no" and not (row.get("entity_search_reason") or "").strip():
        errors.append("entity_search_required=no requires entity_search_reason")
    ...
    mapping_count = json_list_length(row.get("mapped_entity_name", ""))
    entity_type_count = json_list_length(row.get("mapped_entity_type", ""))
    if mapping_count > 0 and entity_type_count != mapping_count:
        errors.append("mapped_entity_type must be a JSON list aligned with mapped_entity_name")
    ...
    if mapping_count > 0 and not ((row.get("mapped_entity_url") or "").strip() or (row.get("evidence_urls") or "").strip()):
        errors.append("mapped_entity_name is non-empty but mapped_entity_url/evidence_urls are empty")
    if mapping_count > 0:
        if not evidence_urls:
            errors.append("entity-mapped rows require evidence_urls")
        if not evidence_source_types:
            errors.append("entity-mapped rows require evidence_source_types")
        if has_entity_evidence.lower() in {"", "no", "none", "false"}:
            errors.append("entity-mapped rows require affirmative has_entity_evidence summary")
    if mapping_count == 0 and (row.get("entity_search_required") or "").strip() == "yes":
        if has_entity_evidence.lower() == "yes":
            errors.append("no-entity searched rows cannot use bare yes in has_entity_evidence")
...
def validate_classifier_row(row: dict[str, str], source: Path) -> list[str]:
    ...
    entity_search_required = row.get("entity_search_required")
    if search_tier not in ALLOWED_SEARCH_TIERS:
        errors.append("search_tier uses an invalid value")
    if entity_search_required not in ALLOWED_YES_NO:
        errors.append("entity_search_required must be yes or no")
    if search_tier in {"full", "light"} and entity_search_required != "yes":
        errors.append("full/light search_tier requires entity_search_required=yes")
    if search_tier == "skip_candidate" and entity_search_required != "no":
        errors.append("skip_candidate requires entity_search_required=no")
    if search_tier == "skip_candidate" and row.get("entity_link_likelihood") in {"high", "medium", "unclear"}:
        ...
        for row in rows:
            ...
            if requires_search_guarantee(schedule_row):
                ...
                if row.get("entity_search_required") != "yes":
                    validation_errors.append(
                        f"{classifier_csv}: task_index={task_index}: search-guarantee rerun batches require entity_search_required=yes"
                    )
...
def validate_row_order_against_tasks(
    ...
    for index in range(compare_count):
        ...
        if actual_task_index != expected_task_index:
            expected_token_name = str(task.get("token_name", task.get("company_name", "")))
            actual_token_name = (row.get("token_name") or "").strip()
            validation_errors.append(
                f"{results_csv}: row {index + 2}: expected "
                f"task_index={expected_task_index} token={expected_token_name!r}, found "
                f"task_index={actual_task_index} token={actual_token_name!r}"
            )
    return validation_errors
            ...
            if action == "edit_row":
                ...
                if corrected_row:
                    if expected_task:
                        ...
                        corrected_row["canonical_slug"] = str(expected_task.get("canonical_slug", ""))
                        corrected_row["token_symbol"] = str(expected_task.get("token_symbol", ""))
                        corrected_row["token_name"] = str(expected_task.get("token_name", ""))
                    if classifier_row:
                        for column in ["token_origin_type", "entity_link_likelihood", "entity_search_required"]:
                    ...
                    expected_canonical_slug = (
                        str(expected_task.get("canonical_slug", ""))
                        if expected_task
                        else (row.get("canonical_slug") or "")
                    )
                    if corrected_row.get("canonical_slug") != expected_canonical_slug:
                        validation_errors.append(
                            f"{report}: task_index={task_index}: corrected_result_row_json canonical_slug mismatch"
                        )
                    verifier_entity_names = normalize_json_list_string(row.get("verifier_mapped_entity_name", ""))
                    corrected_entity_names = normalize_json_list_string(corrected_row.get("mapped_entity_name", ""))
                    if verifier_entity_names and corrected_entity_names != verifier_entity_names:
                        validation_errors.append(
                            f"{report}: task_index={task_index}: corrected_result_row_json mapped_entity_name must match verifier_mapped_entity_name"
                        )
    ...
    for schedule_row in schedule_rows:
        ...
        for key, expected_header in [
            ...
            for index, row in enumerate(rows):
                ...
                if index < len(tasks):
                    task = tasks[index]
                    for field in ["task_index", "canonical_slug", "token_symbol", "token_name"]:
...
def main() -> None:
    ...
    if args.fail_on_identity_drift and identity_override_count and not args.allow_partial:
        validation_errors.append(
            ...
            "Workers must preserve task_index/canonical_slug/token_symbol/token_name exactly from tasks.jsonl."
        )
