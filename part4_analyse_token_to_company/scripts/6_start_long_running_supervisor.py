...
DEFAULT_RUNS_PARENT_DIR = PART4_DIR / "agent_runs"
DEFAULT_FINAL_RESULTS_CSV = PART4_DIR / "agent_runs" / "token_company" / "results.csv"
DEFAULT_RUN_PREFIX = "token_company_parallel_eval"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare the next missing token-company batch window and launch the round "
...
def load_run_managed_processes(runs_dir: Path) -> list[dict[str, object]]:
    ...
    try:
        ...
        return []
    managed = payload.get("managed_processes")
...
def managed_entry_running(entry: dict[str, object]) -> bool:
    ...
    pgid = entry.get("pgid")
    return (
        isinstance(pid, int) and is_pid_running(pid)
    ) or (
        isinstance(pgid, int) and is_pgid_running(pgid)
    )
...
def ensure_no_active_job(path: Path, *, allow_parallel: bool) -> None:
    ...
    runs_dir_raw = latest.get("runs_dir")
    if isinstance(runs_dir_raw, str) and runs_dir_raw.strip():
        runs_dir = Path(runs_dir_raw).resolve()
        live_entries = [entry for entry in load_run_managed_processes(runs_dir) if managed_entry_running(entry)]
        if live_entries:
            raise SystemExit(
                "Managed workers from the previous long-running job are still active. "
                f"run_dir={runs_dir} active_entries={len(live_entries)} "
                "Use --allow-parallel only if you intentionally want another concurrent job."
            )
...
def build_systemd_unit_name(*, timestamp: str) -> str:
    ...
    return f"token-company-longrun-{normalized}"
...
def main() -> None:
    ...
    launcher_state = {
        ...
        "latest_job_json": str(latest_job_json),
